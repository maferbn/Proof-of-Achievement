...
describe("ReputationBadge", function () {
  ...
  describe("MINTER_ROLE Management", function () {
    ...
    it("should revert if non-admin tries to grant MINTER_ROLE", async function () {
      await expect(
        badge.connect(unauthorized).grantMinter(relayer.address)
      ).to.be.revertedWith("ReputationBadge: caller does not have DEFAULT_ADMIN_ROLE");
    });
    ...
    it("should revert if non-admin tries to revoke MINTER_ROLE", async function () {
      ...
      await expect(
        badge.connect(unauthorized).revokeMinter(relayer.address)
      ).to.be.revertedWith("ReputationBadge: caller does not have DEFAULT_ADMIN_ROLE");
    });
  });
  ...
  describe("Minting", function () {
    ...
    it("should assign correct tokenId and tokenURI", async function () {
      const tx = await badge.connect(relayer).mint(user1.address, TEST_URI);
    ...
    it("should revert if unauthorized address tries to mint", async function () {
      await expect(
        badge.connect(unauthorized).mint(user1.address, TEST_URI)
      ).to.be.revertedWith("ReputationBadge: caller does not have MINTER_ROLE");
    });
  ...
  describe("Soulbound (Non-transferible)", function () {
    beforeEach(async function () {
      await badge.connect(owner).grantMinter(relayer.address);
      await badge.connect(relayer).mint(user1.address, TEST_URI);
    });
    ...
    it("should revert when third party tries to transfer user's badge", async function () {
      // First, approve user2 to transfer
    ...
    it("should allow badge to be burned (owner can call burn-like behavior)", async function () {
